...
| Refonte tableau de bord (accueil) | Hero strip (objectif hebdo + CTA séance + timer), bande KPIs 4 tuiles i18n, ligne insights (sparkline 7 jours + donut muscle semaine), séances récentes avec Badge, onboarding premier lancement, locale-aware dates/nombres, a11y charts. | `src/pages/index.astro` (+ fr/en shim auto), `src/i18n/{fr,en}/dashboard.ts` |
